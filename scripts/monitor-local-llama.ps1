<#
Monitor local-llama role-listed processes from .continue/local-llama-processes.json.
Writes a JSON snapshot to .continue/local-llama-monitor.json and appends CSV to logs/local-llama-monitor.csv.
#>
param(
    [int]$SampleIntervalSeconds = 5,
    [int]$SampleCount = 1,
    [string]$MapFile = '.\.continue\local-llama-processes.json',
    [string]$OutJson = '.\.continue\local-llama-monitor.json',
    [string]$OutCsv = '.\\logs\\local-llama-monitor.csv'
)
